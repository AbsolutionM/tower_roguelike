...
## Kleinste Höhe einer Schaltfläche. Im 720-Pixel-Raster sind das auf einem
## 6-Zoll-Handy rund 46 Punkt - knapp über dem, was ein Daumen zuverlässig
## trifft. Alles darunter verfehlt man beim Laufen.
const TOUCH_HEIGHT := 84.0
## Schriften sind für den Bildschirm am Schreibtisch entworfen worden. Auf
## dem Handy entspricht ein Rasterpixel gut einem halben Punkt - ohne diesen
## Faktor wäre Fließtext dort sieben Punkt groß.
const FONT_SCALE := 1.3

static func font_size(base: int) -> int:
	return int(round(float(base) * FONT_SCALE))

## Beschriftung, die in einer engen Zeile nachgibt: sie kürzt sich mit
## Auslassungspunkten, statt ihre Nachbarn aus dem Bild zu schieben. Für die
## dehnbare Spalte einer Zeile - nicht für Zahlen am rechten Rand.
static func make_flex_label(text: String, size: int = 22, color: Color = TEXT) -> Label:
...
## Ränder, die Kerbe und Home-Indikator des Geräts verdecken, in
## Viewport-Pixeln: x = oben, y = unten. Am Schreibtisch beides null.
static func safe_insets(viewport: Viewport) -> Vector2:
	...
	# Ist die sichere Fläche größer als das Fenster, liegt kein Handy vor.
	if safe.size.y <= 0 or safe.size.y >= window.y:
		return Vector2.ZERO
	var scale: float = viewport.get_visible_rect().size.y / float(window.y)
	return Vector2(
		maxf(float(safe.position.y) * scale, 0.0),
		maxf(float(window.y - safe.end.y) * scale, 0.0)
	)
...
static func make_label(text: String, size: int = 22, color: Color = TEXT, alignment: int = HORIZONTAL_ALIGNMENT_LEFT, wrap: bool = false) -> Label:
	var label := Label.new()
	label.text = text
...
static func make_button(text: String, size: int = 22, accent: Color = ACCENT) -> Button:
	var button := Button.new()
	button.text = text
	button.add_theme_font_size_override("font_size", font_size(size))
...
static func make_primary_button(text: String, size: int = 26, accent: Color = ACCENT) -> Button:
	# Gefüllt, aber abgedunkelt - ein voll gesättigter Balken über die ganze
	# Breite erschlägt sonst den Rest des Bildschirms.
	var button := make_button(text, size, accent)
	button.custom_minimum_size = Vector2(0.0, TOUCH_HEIGHT + 16.0)
...
static func make_screen_root(parent: Control, margin: int = 28) -> VBoxContainer:
	...
	# Kerbe und Home-Indikator kommen zum normalen Rand dazu.
	var insets := safe_insets(parent.get_viewport())
	...
	margin_box.add_theme_constant_override("margin_top", margin + int(insets.x))
	margin_box.add_theme_constant_override("margin_bottom", margin + int(insets.y))
...
static func make_attribute_row(attribute_name: String, value: int, maximum: int, color: Color) -> HBoxContainer:
	...
	var name_label := make_flex_label(attribute_name, 17, TEXT_DIM)
...
static func make_section(title: String, accent: Color = TEXT_DIM) -> Control:
	...
	var title_label := make_label(title, 19, accent)
...
static func make_stat_sheet(entries: Array, columns: int = 2) -> Control:
	...
	for entry in entries:
		grid.add_child(make_flex_label(str(entry["name"]), 15, TEXT_DIM))
